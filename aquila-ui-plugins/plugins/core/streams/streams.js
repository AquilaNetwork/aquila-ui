...
const objectToArray = (object) => {
    ...
    let groupList = object.groups.map(group => group.groupId === 0 ? { groupId: group.groupId, url: `group/${group.groupId}`, groupName: "Aquila General Chat", sender: group.sender, senderName: group.senderName, timestamp: group.timestamp === undefined ? 1 : group.timestamp } : { ...group, url: `group/${group.groupId}` })
        ...
        activeChatSocket.onclose = () => {
            ...
            if (closeGracefully === false && initial <= 52) {
                ...
                if (initial <= 52) {
                    ...
                    parentEpml.request('showSnackBar', "Connection to the Aquila Core was lost, is your Core running ?")
                    retryOnClose = true
                    setTimeout(pingActiveChatSocket, 10000)
                    initial = initial + 1
                } else {

                    parentEpml.request('showSnackBar', "Cannot connect to the Aquila Core, restart UI and Core!")
                }
            }
        }
