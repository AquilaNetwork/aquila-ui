...
import nacl from '../../../../aquila-ui-crypto/api/deps/nacl-fast.js'
import Base58 from '../../../../aquila-ui-crypto/api/deps/Base58.js'
import publicKeyToAddress from '../../../../aquila-ui-crypto/api/wallet/publicKeyToAddress.js'
...
class Puzzles extends LitElement {
        ...
        return html`
			<div id="puzzle-page">
				<div style="min-height:48px; display: flex; padding-bottom: 6px;">
				    ...
				    <vaadin-grid theme="compact, wrap-cell-content" id="puzzlesGrid" ?hidden="${this.isEmptyArray(this.puzzles)}" .items="${this.puzzles}" aria-label="Puzzles" all-rows-visible>
				        <vaadin-grid-column width="20em" header="${translate("puzzlepage.pchange2")}" .renderer=${(root, column, data) => {
                            			if (data.item.isSolved) {
                                			...
                                			render(html`<span>${data.item.reward} UNCIA</span>`, root)
                            			}
                        		}}>
                                        </vaadin-grid-column>
					...
					<div>${translate("puzzlepage.pchange9")} ${this.selectedPuzzle.reward} UNCIA:</div>
					<br>
    ...
    async submitPuzzleGuess(e) {
        ...
        // Mostly copied from aquila-ui-core/src/plugins/routes.js
