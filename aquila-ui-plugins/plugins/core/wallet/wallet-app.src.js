...
import '../components/AquilaQrcodeGenerator.js'
...
const coinsNames = ['uncia', 'btc', 'ltc', 'doge', 'dgb', 'rvn', 'arrr']
...
class MultiWallet extends LitElement {
    ...
    static get styles() {
        return css`
            * {
            ...
            .uncia .currency-image {
                background-image: url('/img/uncia.png');
            }
    ...
    constructor() {
        ...
        this.wallets.get('uncia').wallet = window.parent.reduxStore.getState().app.selectedAddress
        ...
        parentEpml.ready().then(() => {
            parentEpml.subscribe('selected_address', async (selectedAddress) => {
                ...
                this.wallets.get('uncia').wallet = selectedAddress
                this.wallets.get('btc').wallet = window.parent.reduxStore.getState().app.selectedAddress.btcWallet
                this.wallets.get('ltc').wallet = window.parent.reduxStore.getState().app.selectedAddress.ltcWallet
                this.wallets.get('doge').wallet = window.parent.reduxStore.getState().app.selectedAddress.dogeWallet
                this.wallets.get('dgb').wallet = window.parent.reduxStore.getState().app.selectedAddress.dgbWallet
        ...
        return html`
            <div class="wrapper">
                ...
                <mwc-tab-bar id="tabs-1" activeIndex="0">
                    <mwc-tab label="Aquila" hasImageIcon minWidth @click="${(e) => this.tabWalletUncia()}">
                        <img slot="icon" width="24px" height="24px" src="/img/uncia.png">
                    </mwc-tab>
                ...
                <div class="transactions-wrapper">
                    ...
                    <div class="qrcode-pos" ?hidden="${this.getSelectedWalletAddress().length < 1}">
                        <aquila-qrcode-generator data="${this.getSelectedWalletAddress()}" mode="octet" format="html" auto></aquila-qrcode-generator>
                    </div>
                    <div id="transactions">
                ...
                <mwc-dialog id="showTransactionDetailsDialog" scrimClickAction="${this.showTransactionDetailsLoading ? '' : 'close'}">
                    ...
                    <div id="transactionList">
                        ...
                        ${!this.selectedTransaction.amount ? '' : html`
                            ...
                            <br />
                            <div><span>${this.selectedTransaction.amount} UNCIA</span></div>
                        `}
                ...
                <mwc-dialog id="sendUnciaDialog" scrimClickAction="" escapeKeyAction="">
                    <div class="send-coin-dialog">
                        <div style="text-align: center;">
                            <img src="/img/uncia.png" width="32" height="32">
                            <h2> ${translate("walletpage.wchange17")} UNCIA</h2>
                            <hr />
                        </div>
                        <p>
                        ...
                        <p>
                            ...
                            <span style="float: left; font-weight: bold; display: inline;">
                                <vaadin-button theme="primary small" style="width: 100%;" @click=${() => this.calculateAquilal()}><vaadin-icon icon="vaadin:coin-piles" slot="prefix"></vaadin-icon> ${translate("walletpage.wchange45")} UNCIA</vaadin-button>
                            </span><br /><span>&nbsp;</span>
                        </p>
                        <p>
                            <mwc-textfield
                                style="width: 100%;"
                                required
                                @input="${(e) => { this.checkUnciaAmount(e) }}"
                                id="amountInput"
                                label="${translate("walletpage.wchange11")} (UNCIA)"
                                type="number"
                                auto-validate="false"
                                value="${this.amount}"
                            >
                        ...
                        <div style="margin-bottom: 10px;">
                            <p style="margin-bottom: 0;">${translate("walletpage.wchange21")} <span style="font-weight: bold;">0.001 UNCIA<span></p>
                        </div>
                        ...
                        <div class="buttons">
                            <div>
                                <vaadin-button ?disabled="${this.btnDisable}" theme="primary medium" style="width: 100%;" @click=${() => this.sendUncia()}>
                                    <vaadin-icon icon="vaadin:arrow-forward" slot="prefix"></vaadin-icon>
                                    ${translate("walletpage.wchange17")} UNCIA
                                </vaadin-button>
                                <div style="display:${this.unciaWarning ? 'inline' : 'none'}">${this.renderWarning()}</div>
                            </div>
                        </div>
                    </div>
                    <mwc-button
                        slot="primaryAction"
                        @click="${() => this.closeUnciaDialog()}"
                        class="red"
                    >
                    ${translate("general.close")}
                    </mwc-button>
                ...
                <mwc-dialog id="unciaBookDialog">
                    <div style="text-align:center">
                        <img src="/img/uncia.png" width="32" height="32">
                        <h1>Aquila ${translate("walletpage.wchange47")}</h1>
                    </div>
                    ...
                    <hr>
                    <br>
                    <vaadin-grid theme="compact" id="unciaBookGrid" ?hidden="${this.isEmptyArray(this.unciaBook)}" aria-label="UNCIA Addressbook" .items="${this.unciaBook}" all-rows-visible>
                        ...
                        <vaadin-grid-column width="11rem" flex-grow="0" header="${translate("chatpage.cchange13")}" .renderer=${(root, column, data) => {
                            render(html`${this.renderSendFromUnciaAddressbookButton(data.item)}`, root);
                        }}>
                        </vaadin-grid-column>
                    </vaadin-grid>
                    ${this.isEmptyArray(this.unciaBook) ? html`
                    ...
                    <mwc-button
                         ...
                         @click=${() => this.openAddToUnciaAddressbook()}
                    >
                ...
                <mwc-dialog id="addUnciaAddressDialog" scrimClickAction="" escapeKeyAction="">
                    <div style="text-align:center">
                        <img src="/img/uncia.png" width="32" height="32">
                        <h1>Aquila ${translate("walletpage.wchange47")}</h1><br />
                    ...
                    <div style="min-height: 150px; min-width: 500px; box-sizing: border-box; position: relative;">
                        ...
                        <p>
                            <mwc-textfield
                                style="width: 100%;"
                                required
                                id="unciaAddressInput"
                                label="${translate("login.address")}"
                                type="text"
                                value="${this.unciaBookAddress}"
                            >
                            </mwc-textfield>
                        </p>
                    </div>
                    <div class="buttons">
                        <div>
                            <vaadin-button theme="primary medium" style="width: 100%;" @click=${() => this.addToAquilaAddressbook()}>
                    ...
                    <mwc-button
                        slot="primaryAction"
                        @click="${() => this.closeUnciaAddressDialog()}"
                        class="red"
                    >
                    ${translate("general.close")}
                    </mwc-button>
                ...
                <mwc-dialog id="importUnciaAddressbookDialog">
                    <div style="text-align:center">
                        <img src="/img/uncia.png" width="32" height="32">
                        <h1>Aquila ${translate("walletpage.wchange53")}</h1><br />
                        <hr>
                        <br>
                    </div>
                    <div style="min-height: 150px; min-width: 500px; box-sizing: border-box; position: relative;">
                        <frag-file-input accept=".uncia.json" @file-read-success="${(e) => this.importUnciaAddressbook(e.detail.result)}"></frag-file-input>
    ...
    renderWarning() {
        return html`<span class="warning-text">${translate("tradepage.tchange48")} UNCIA</span>`
    }
    ...
    unciaAddressbook() {
        if (localStorage.getItem("addressbookUncia") === null) {
            localStorage.setItem("addressbookUncia", "")
        } else {
            this.unciaBook = JSON.parse(localStorage.getItem("addressbookUncia") || "[]")
        }
    }
    ...
    openUnciaAddressbook() {
        this.shadowRoot.querySelector("#unciaBookDialog").show()
    }
    ...
    openAddUnciaAddressDialog() {
        this.unciaBookAddress = this.selectedTransaction.recipient
        this.openAddToUnciaAddressbook()
    ...
    openAddToUnciaAddressbook() {
        this.shadowRoot.querySelector("#addUnciaAddressDialog").show()
    }
    ...
    openImportUnciaAddressbook() {
        this.shadowRoot.querySelector("#importUnciaAddressbookDialog").show()
    }
    ...
    closeUnciaAddressDialog() {
        this.shadowRoot.querySelector('#addUnciaAddressDialog').close()
        this.shadowRoot.getElementById('unciaNameInput').value = ''
        this.shadowRoot.getElementById('unciaAddressInput').value = ''
        this.unciaBookName = ''
        this.unciaBookAddress = ''
    }
    ...
    closeImportUnciaAddressbookDialog() {
        this.shadowRoot.querySelector("#importUnciaAddressbookDialog").close()
    }
    ...
    addToAquilaAddressbook() {
        let name = this.shadowRoot.getElementById('unciaNameInput').value
        let address = this.shadowRoot.getElementById('unciaAddressInput').value

        var oldAquilaBook = JSON.parse(localStorage.getItem("addressbookUncia") || "[]")

        if (name.length === 0) {
            let unciabookstring1 = get("walletpage.wchange50")
            parentEpml.request('showSnackBar', `${unciabookstring1}`)
            return false
        }

        if (address.length === 0) {
            let unciabookstring2 = get("walletpage.wchange51")
            parentEpml.request('showSnackBar', `${unciabookstring2}`)
            return false
        }

        const newAquilaBookItem = {
            name: name,
            address: address
        }

        oldAquilaBook.push(newAquilaBookItem)

        localStorage.setItem("addressbookUncia", JSON.stringify(oldAquilaBook))

        let unciabookstring2 = get("walletpage.wchange52")
        parentEpml.request('showSnackBar', `${unciabookstring2}`)

        this.closeUnciaAddressDialog()
        this.unciaBook = JSON.parse(localStorage.getItem("addressbookUncia") || "[]")
    }
    ...
    sendFromUnciaAddressbook(websiteObj) {
        let address = websiteObj.address
        this.recipient = address
        this.openSendUncia()
        this.shadowRoot.querySelector('#unciaBookDialog').close()
    }
    ...
    renderSendFromUnciaAddressbookButton(websiteObj) {
        return html`<mwc-button dense unelevated label="${translate("walletpage.wchange17")} UNCIA" icon="send" @click="${() => this.sendFromUnciaAddressbook(websiteObj)}"></mwc-button>`
    }
    ...
    exportUnciaAddressbook() {
        const unciaBookData = JSON.stringify(localStorage.getItem("addressbookUncia"))
        const unciaBookSave = JSON.parse((unciaBookData) || "[]")
        const blob = new Blob([unciaBookSave], { type: 'text/plain;charset=utf-8' })
        FileSaver.saveAs(blob, `aquila_addressbook.uncia.json`)
    }
    ...
    importUnciaAddressbook(file) {
        localStorage.removeItem("addressbookUncia")
        const newItems = JSON.parse((file) || "[]")
        localStorage.setItem("addressbookUncia", JSON.stringify(newItems))
        this.unciaBook = JSON.parse(localStorage.getItem("addressbookUncia") || "[]")
        this.shadowRoot.querySelector('#importUnciaAddressbookDialog').close()
    }
    ...
    closeUnciaDialog() {
        this.shadowRoot.querySelector('#sendUnciaDialog').close()
    ...
    renderSQB() {
        ...
        if (displaybutton == null) {
            ...
            return html`
                ...
                <paper-icon-button icon="icons:send" @click=${() => this.sendToUnciaAddress()} title="${translate("walletpage.wchange46")}"></paper-icon-button>
                <paper-icon-button icon="icons:add-circle" @click=${() => this.openAddUnciaAddressDialog()} title="${translate("walletpage.wchange49")}"></paper-icon-button>
            `
        }
    }
    ...
    checkUnciaAmount(e) {
        ...
        if (targetAmount.length === 0) {
            ...
            const checkUnciaAmountInput = this.shadowRoot.getElementById('amountInput').value
            const checkUnciaAmount = this.round(parseFloat(checkUnciaAmountInput))
            const myFunds = this.round(parseFloat(this.balance - 0.00110000))
            if (Number(myFunds) >= Number(checkUnciaAmount)) {
                this.shadowRoot.getElementById('amountInput').value = checkUnciaAmountInput
                this.btnDisable = false
                this.unciaWarning = false
            } else {
                this.shadowRoot.getElementById('amountInput').value = checkUnciaAmountInput
                this.btnDisable = true
                this.unciaWarning = true
            }
        }
        ...
        switch (coin) {
            case 'uncia':
                ...
                parentEpml.request('apiCall', {
                    url: `/addresses/balance/${this.wallets.get('uncia').wallet.address}?apiKey=${this.getApiKey()}`,
                })
                .then((res) => {
                    if (isNaN(Number(res))) {
    ...
    renderSendButton() {
        if ( this._selectedWallet === "uncia" ) {
            return html`<vaadin-button theme="primary medium" style="width: 100%;" @click=${() => this.openSendUncia()}><vaadin-icon icon="vaadin:coin-piles" slot="prefix"></vaadin-icon> ${translate("walletpage.wchange17")} UNCIA</vaadin-button>`
    ...
    renderAddressbookButton() {
        if ( this._selectedWallet === "uncia" ) {
            return html`<vaadin-button theme="primary medium" style="width: 100%;" @click=${() => this.openUnciaAddressbook()}><vaadin-icon icon="vaadin:book" slot="prefix"></vaadin-icon> ${translate("walletpage.wchange47")}</vaadin-button>`
        } else if ( this._selectedWallet === "btc" ) {
    ...
    renderExportAddressbookButton() {
        if ( this._selectedWallet === "uncia" ) {
            return html`<vaadin-button theme="primary small" style="width: 100%;" @click=${() => this.exportUnciaAddressbook()}><vaadin-icon icon="vaadin:cloud-download" slot="prefix"></vaadin-icon> ${translate("walletpage.wchange54")}</vaadin-button>`
        } else if ( this._selectedWallet === "btc" ) {
    ...
    renderImportAddressbookButton() {
        if ( this._selectedWallet === "uncia" ) {
            return html`<vaadin-button theme="primary small" style="width: 100%;" @click=${() => this.openImportUnciaAddressbook()}><vaadin-icon icon="vaadin:cloud-upload" slot="prefix"></vaadin-icon> ${translate("walletpage.wchange53")}</vaadin-button>`
        } else if ( this._selectedWallet === "btc" ) {
    ...
    openSendUncia() {
        this.shadowRoot.querySelector("#sendUnciaDialog").show();
    }
    ...
    async getTransactionGrid(coin) {
        ...
        if (coin === 'uncia') {
            this.transactionsGrid.addEventListener(
                'click',
                (e) => {
                    let unciaItem = this.transactionsGrid.getEventContext(e).item
                    this.showUnciaTransactionDetails(unciaItem, this.wallets.get(this._selectedWallet).transactions)
                },
                { passive: true }
            )
        } else if (coin === 'btc') {
    ...
    async renderTransactions() {
        if (this._selectedWallet === 'uncia') {
            render(this.renderUnciaTransactions(this.wallets.get(this._selectedWallet).transactions, this._selectedWallet), this.transactionsDOM)
        } else if (this._selectedWallet === 'btc') {
    ...
    renderUnciaTransactions(transactions, coin) {
        ...
        return html`
            ...
            <vaadin-grid theme="large" id="${coin}TransactionsGrid" ?hidden="${this.isEmptyArray(this.wallets.get(this._selectedWallet).transactions)}" page-size="25" all-rows-visible>
                ...
                <vaadin-grid-column
                    ...
                    .renderer=${(root, column, data) => {
                        render(html` ${data.item.type} ${data.item.creatorAddress === this.wallets.get('uncia').wallet.address ? html`<span class="color-out">${translate("walletpage.wchange7")}</span>` : html`<span class="color-in">${translate("walletpage.wchange8")}</span>`} `, root)
                    }}
                >
                </vaadin-grid-column>
                <vaadin-grid-column
    ...
    showUnciaTransactionDetails(myTransaction, allTransactions) {
        allTransactions.forEach((transaction) => {
            if (myTransaction.signature === transaction.signature) {
                let txnFlow = myTransaction.creatorAddress === this.wallets.get('uncia').wallet.address ? 'OUT' : 'IN'
