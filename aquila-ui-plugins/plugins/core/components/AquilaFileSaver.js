function saveFile({ data, debug, filename }) {
  if (debug) console.log("[aquila-file-saver] starting with ", { data, debug, filename });
  if (!data) throw new Error("[aquila-file-saver] You must pass in data");
  if (!filename) {
    if (typeof window !== "undefined" && typeof File !== undefined && data instanceof File) {
      filename = data.name;
    }
    throw new Error("[aquila-file-saver] You must pass in filename");
  }
  ...
  function convertImageToCanvas({ debug, img }) {
    if (debug) console.log("[aquila-file-saver] starting convertImageToCanvas");
  ...
  function saveHTML({ data, debug, filename }) {
    if (typeof data === "object" && "outerHTML" in data) {
      if (debug) console.log("[aquila-file-saver] data appears to be an HTML element, so grabbing it's outer HTML");
      data = data.outerHTML;
    }
  ...
  function saveDSV({ data, debug, delimiter, filename, mediatype }) {
    if (!Array.isArray(data)) throw new Error("[aquila-saver] data must be an array to save as a CSV");
    if (!delimiter) throw new Error("[aquila-saver] delimiter must be set");
    if (!mediatype) throw new Error("[aquila-saver] mediatype must be set");
  ...
  function saveBlob({ data, debug, filename }) {
    ...
    if (debug) console.log("[aquila-file-saver.saveBlob] url:", url);
    saveDataOrBlobURL({ url, debug, filename });
  ...
  if (ext === ".csv") {
    ...
    throw new Error('[aquila-file-saver] unrecognized extension "' + ext + '"');
  }
}
