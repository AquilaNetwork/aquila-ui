...
class AquilaInfoView extends LitElement {
        ...
        return html`
            <paper-dialog class="full-info-wrapper" id="userFullInfoDialog" modal>
                ...
                <div class="data-info" style="display:${this.isLoadingCompleteInfo ? 'none' : ''}">
                    <ul>
                        ...
                        <li>
                            <span class="data-items">${translate("general.balance")}</span> ${this.displayBalance} UNCIA
                        </li>
                        <li>
                ...
                <div class="data-info" style="display:${this.isLoadingCompleteInfo ? 'none' : ''}">
                    <ul>
                        ...
                        <li>
                            <span class="data-items">UNCIA ${translate("explorerpage.exp19")}</span> ${this.totalSent.toFixed(0)} UNCIA
                        </li>
                        <li>
                            <span class="data-items">${translate("explorerpage.exp20")}</span> ${this.allReceivedPayments.length}
                        </li>
                        <li>
                            <span class="data-items">UNCIA ${translate("explorerpage.exp20")}</span> ${this.totalReceived.toFixed(0)} UNCIA
                        </li>
                    </ul>
                </div>
                <div class="explorer-trades">
                    <div class="box-info">
                        ...
                        <div class="border-wrapper">
                            <div class="loadingContainer" id="loadingExplorerTrades" style="display:${this.isLoadingCompleteInfo ? 'block' : 'none'}"><div class="loading"></div><span style="color: var(--black);">${translate("login.loading")}</span></div>
                            <vaadin-grid theme="compact" id="allUnciaPaymentsGrid" ?hidden="${this.isEmptyArray(this.allPayments)}" .items="${this.allPayments}">
                                <vaadin-grid-column
                                    auto-width
            ...
            <paper-dialog style="background: var(--white); border: 1px solid var(--black); border-radius: 5px;" id="showTxDetailsDialog" modal>
                ...
                <div id="transactionList">
                    ...
                    <br />
                    <div><span>${this.txfee} UNCIA</span></div>

                    <span class="title">${translate("walletpage.wchange37")}</span>
                    <br />
                    <div><span>${this.txamount} UNCIA</span></div>
    ...
    async getAddressFromName(fromName) {
        ...
        const aquilaNameInfo = await fetch(fromNameUrl).then(response => {
            return response.json()
        })

        this.nameAddressResult = aquilaNameInfo
    ...
    async getAddressUserInfo(infoAddress) {
        ...
        const aquilaAddressInfo = await fetch(infoAddressUrl).then(response => {
            return response.json()
        })

        this.addressResult = aquilaAddressInfo
    }
    ...
    async getAddressUserAvatar(avatarAddress) {
        ...
        const myImageUrl = `${nodeUrl}/arbitrary/THUMBNAIL/${this.imageName}/aquila_avatar?async=true&apiKey=${this.getApiKey()}`
    ...
    async getAddressUserBalance(balanceAddress) {
        ...
        const aquilaBalanceInfo = await fetch(balanceAddressUrl).then(res => {
            return res.json()
        })

        this.displayBalance = aquilaBalanceInfo.toFixed(4)
    }
...
window.customElements.define('aquila-info-view', AquilaInfoView)
