...
class TradePortal extends LitElement {
			...
			<div class="box">
				...
				<div class="border-wrapper">
					...
					<vaadin-grid theme="compact column-borders row-stripes wrap-cell-content" id="historicTradesGrid" aria-label="Historic Trades" .items="${this.listedCoins.get(this.selectedCoin).historicTrades}">
						<vaadin-grid-column auto-width resizable header="${translate("tradepage.tchange8")} (UNCIA)" path="unciaAmount"></vaadin-grid-column>
						<vaadin-grid-column
							auto-width
							resizable
				...
				<div class="border-wrapper">
					...
					<vaadin-grid multi-sort="true" theme="compact column-borders row-stripes wrap-cell-content" id="openOrdersGrid" aria-label="Open Orders" .items="${this.listedCoins.get(this.selectedCoin).openFilteredOrders}">
						<vaadin-grid-column
							...
							.renderer=${(root, column, data) => {
								render(html`<span @click="${() => this.fillBuyForm(data)}">${this.round(data.item.unciaAmount)}</span>`, root)
							}}
						>
						</vaadin-grid-column>
						<vaadin-grid-column
						...
						<vaadin-grid-column
							...
							.renderer=${(root, column, data) => {
								render(html`<span @click="${() => this.fillBuyForm(data)}">${data.item.aquilaCreator}</span>`, root)
							}}
						>
						</vaadin-grid-column>
						<vaadin-grid-column
							...
							.renderer=${(root, column, data) => {
								render(html`<span @click="${() => this.getAllForAddress(data.item.aquilaCreator)}"><mwc-icon class="btn-info">info</mwc-icon></span>`, root)
							}}
						>
						</vaadin-grid-column>
					</vaadin-grid>
		...
		<div class="open-market-container">
			<div class="box">
				<mwc-tab-bar id="tabs-1" activeIndex="0">
					<mwc-tab id="tab-buy" label="${translate("tradepage.tchange18")} UNCIA" @click="${(e) => this.displayTabContent('buy')}"></mwc-tab>
					<mwc-tab id="tab-sell" label="${translate("tradepage.tchange19")} UNCIA" @click="${(e) => this.displayTabContent('sell')}"></mwc-tab>
				</mwc-tab-bar>
				<z id="tabs-1-content">
					...
					<div id="tab-sell-content">
						<div class="card">
							...
							<span class="amt-text">
                                              <span class="balance-text">${translate("tradepage.tchange16")}: ${this.listedCoins.get("AQUILA").balance} UNCIA</span>
							    <span class="fee-text">${translate("walletpage.wchange12")}: ${this.listedCoins.get("AQUILA").tradeFee} UNCIA</span>
                                          </span>
							<div class="buttons">
								<div>
    ...
    render() {
		...
		<div id="trade-portal-page">
			<div style="min-height: 40px; display: flex; padding-bottom: 0px; margin: 2px 2px 0px 2px ;">
				<h2 style="margin: 0 0 15px 0; line-height: 50px; display: inline;">Aquila ${translate("tradepage.tchange1")} - &nbsp;</h2>
				<mwc-select outlined id="coinSelectionMenu" label="${translate("tradepage.tchange2")}">
					<mwc-list-item value="LITECOIN" selected><span class="coinName ltc" style="color: var(--black);">UNCIA / LTC</span></mwc-list-item>
					<mwc-list-item value="BITCOIN"><span class="coinName btc" style="color: var(--black);">UNCIA / BTC</span></mwc-list-item>
					<mwc-list-item value="DOGECOIN"><span class="coinName doge" style="color: var(--black);">UNCIA / DOGE</span></mwc-list-item>
					<mwc-list-item value="DIGIBYTE"><span class="coinName dgb" style="color: var(--black);">UNCIA / DGB</span></mwc-list-item>
					<mwc-list-item value="RAVENCOIN"><span class="coinName rvn" style="color: var(--black);">UNCIA / RVN</span></mwc-list-item>
					<mwc-list-item value="PIRATECHAIN"><span class="coinName arrr" style="color: var(--black);">UNCIA / ARRR</span></mwc-list-item>
				</mwc-select>
		...
		<mwc-dialog style="background: var(--white);" id="sellerDialog">
			<div class="card-container">
				...
				<p>${translate("explorerpage.exp2")}: ${this.displayTradeBalance} UNCIA</p>
			</div>
			<mwc-button slot="primaryAction" dialogAction="cancel" class="cancel">${translate("general.close")}</mwc-button>
		</mwc-dialog>
    ...
    setDefaultSellPrice() {
        ...
        tabSellPrice.value = this.unciaRatio.isNaN ? 0 : this.unciaRatio
    }
    ...
    fillBuyForm(sellerRequest) {
        this.shadowRoot.getElementById('buyAmountInput').value = parseFloat(sellerRequest.item.unciaAmount)
        this.shadowRoot.getElementById('buyPriceInput').value = this.round(parseFloat(sellerRequest.item.foreignAmount) / parseFloat(sellerRequest.item.unciaAmount))
        this.shadowRoot.getElementById('buyTotalInput').value = parseFloat(sellerRequest.item.foreignAmount)
        this.shadowRoot.getElementById('aquilaAtAddress').value = sellerRequest.item.aquilaAtAddress
    ...
    async getAddressTradeInfo(tradeInfoAddress) {
        ...
        const aquilaTradeAddressInfo = await fetch(tradeInfoAddressUrl).then(response => {
            return response.json()
        })

        this.tradeAddressResult = aquilaTradeAddressInfo
    }
    ...
    async getAddressTradeAvatar(tradeAvatarAddress) {
        ...
        const myTradeImageUrl = `${nodeUrl}/arbitrary/THUMBNAIL/${this.tradeImageName}/aquila_avatar?async=true&apiKey=${this.getApiKey()}`
    ...
    async getAddressTradeBalance(tradeBalanceAddress) {
        ...
        const aquilaTradeBalanceInfo = await fetch(tradeBalanceAddressUrl).then(res => {
            return res.json()
        })
        this.displayTradeBalance = aquilaTradeBalanceInfo
    }
    ...
    *   - when this happens, Alice uses secret-B and secret-A to redeem the UNCIA from the AT, so until Alice does this, Bob's trade-bot state is BOB_WAITING_FOR_AT_REDEEM
    *   - after Alice redeems UNCIA from AT, Bob can extract secret-A and capture the actual LTC funds from P2SH-A to his own litecoin account and hence his trade-bot moves to BOB_DONE
    ...
    *   - if Bob spends P2SH-B, then Alice can send secret-B and secret-A to the AT, claim the UNCIA and she's ALICE_DONE
    ...
    */
    ...
    async sellAction() {
        ...
        const fundingUnciaAmount = this.round(parseFloat(sellAmountInput) + 0.001)
        ...
        const makeRequest = async () => {
            ...
            const response = await parentEpml.request('tradeBotCreateRequest', {
                ...
                unciaAmount: parseFloat(sellAmountInput),
                fundingUnciaAmount: parseFloat(fundingUnciaAmount),
        ...
        if (this.round(parseFloat(fundingUnciaAmount) + parseFloat(0.002)) > parseFloat(this.listedCoins.get("AQUILA").balance)) {
            this.isSellLoading = false
            this.sellBtnDisable = false
    ...
    async buyAction() {
        ...
        this.buyBtnDisable = true
        const aquilaAtAddress = this.shadowRoot.getElementById('aquilaAtAddress').value
        let _foreignKey = ""
        ...
        const manageResponse = (response) => {
            if (response === true) {
                ...
                this.shadowRoot.getElementById('aquilaAtAddress').value = ''
    ...
    async cancelStuckOfferAction(offer) {
        ...
        const makeRequest = async () => {
            const response = await parentEpml.request('deleteTradeOffer', {
                creatorPublicKey: this.selectedAddress.base58PublicKey,
                atAddress: offer.aquilaAtAddress,
            })
            return response
        }
    ...
    renderCancelStuckOfferButton(offerItem) {
        if (offerItem.mode === 'OFFERING' && offerItem.aquilaCreator === this.selectedAddress.address) {
            return html`<mwc-button id="offer-${offerItem.aquilaAtAddress}" ?disabled=${this.cancelStuckOfferBtnDisable} class="cancel" @click=${(e) => this.cancelStuckOfferAction(offerItem)}>CANCEL</mwc-button>`
        } else {
            return ''
        }
    }
    ...
    handleStuckTrades() {
        ...
        const filterStuckOffers = (myOffers) => {
            ...
            const stuckOffers = myOffers.filter((myOffer) => {
                let value = myTradeBotStates.some((myTradeBotState) => myOffer.aquilaAtAddress === myTradeBotState.atAddress)
                return !value
            })
            return stuckOffers
        }
        ...
        const getOffers = async () => {
            ...
            const myOpenTradeOrders = await openTradeOrders.filter((order) => order.mode === 'OFFERING' && order.aquilaCreator === 'SELECTED_ADDRESS')
    ...
    filterStuckTrades(states) {
        ...
        const doStuckOffers = (message) => {
            ...
            const offerItem = (offer) => {
                return {
                    ...offer,
                    price: this.round(parseFloat(offer.expectedForeignAmount) / parseFloat(offer.unciaAmount)),
                }
            }
            const addStuckOrders = (offerItem) => {
                if (offerItem.aquilaCreator === this.selectedAddress.address) {
