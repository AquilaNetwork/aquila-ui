...
class TradeBotPortal extends LitElement {
    static get properties() {
        return {
            ...
            unciabtc: { type: Number },
            uncialtc: { type: Number },
            unciadoge: { type: Number },
            unciadgb: { type: Number },
            unciarvn: { type: Number },
            unciaarrr: { type: Number },
            btcuncia: { type: Number },
            ltcuncia: { type: Number },
            dogeuncia: { type: Number },
            dgbuncia: { type: Number },
            rvnuncia: { type: Number },
            arrruncia: { type: Number },
    ...
    static get styles() {
		...
		* {
		...
		.btc.coinName:before  {
			background-image: url('/img/unciabtc.png');
		}
		.ltc.coinName:before  {
			background-image: url('/img/uncialtc.png');
		}
		.doge.coinName:before  {
			background-image: url('/img/unciadoge.png');
		}
		.dgb.coinName:before  {
			background-image: url('/img/unciadgb.png');
		}
		.rvn.coinName:before  {
			background-image: url('/img/unciarvn.png');
		}
            .arrr.coinName:before  {
			background-image: url('/img/unciaarrr.png');
		}
		.coinName {
			display: inline-block;
			height: 26px;
    ...
    constructor() {
	  ...
	  let ravencoin = {
        ...
        this.listedCoins.set("AQUILA", aquila)
				...
				<div class="border-wrapper">
					...
					<vaadin-grid multi-sort="true" theme="compact column-borders row-stripes wrap-cell-content" id="openOrdersGrid" aria-label="Open Orders" .items="${this.listedCoins.get(this.selectedCoin).openFilteredOrders}">
						<vaadin-grid-column
							auto-width
							resizable
							header="${translate("tradepage.tchange8")} (UNCIA)"
							id="unciaAmountColumn"
							.renderer=${(root, column, data) => {
								render(html`<span>${this.round(data.item.unciaAmount)}</span>`, root)
							}}
						>
						</vaadin-grid-column>
						<vaadin-grid-column
						...
						<vaadin-grid-column
							...
							id="aquilaCreatorColumn"
							.renderer=${(root, column, data) => {
								render(html`<span>${data.item.aquilaCreator}</span>`, root)
							}}
						>
						</vaadin-grid-column>
					</vaadin-grid>
				...
				<div class="border-wrapper">
					...
					<vaadin-grid multi-sort="true" theme="compact column-borders row-stripes wrap-cell-content" id="myOrdersGrid" aria-label="My Orders" .items="${this.listedCoins.get(this.selectedCoin).myOrders}">
						...
						<vaadin-grid-column
							...
							header="${translate("tradepage.tchange9")} (${this.listedCoins.get(this.selectedCoin).coinCode})"
							.renderer=${(root, column, data) => {
								const price = this.round(parseFloat(data.item.foreignAmount) / parseFloat(data.item.unciaAmount))
						...
						<vaadin-grid-column
							auto-width
							resizable
							header="${translate("tradepage.tchange8")} (UNCIA)" path="unciaAmount"
						>
						</vaadin-grid-column>
						<vaadin-grid-column
							auto-width
		...
		<div class="my-historic-trades">
			<div class="box">
				...
				<div class="border-wrapper">
					...
					<vaadin-grid theme="compact column-borders row-stripes wrap-cell-content" id="myDoneTradesGrid" aria-label="My Buyed Aquila" .items="${this.listedCoins.get(this.selectedCoin).myGridBoughtItems}">
						<vaadin-grid-column
							auto-width
							resizable
							header="${translate("tradepage.tchange11")}"
    ...
    tradeBotBTCTemplate() {
		...
		<div class="trade-bot-container">
			<div class="box-bot">
				<header><span>${translate("info.inf16")} UNCIA ${translate("info.inf14")} ${this.listedCoins.get(this.selectedCoin).coinCode}</span></header>
				<div class="border-wrapper">
					<vaadin-grid theme="compact column-borders row-stripes wrap-cell-content" id="tradeBotBTCGrid" aria-label="Auto Buy BTC" ?hidden="${this.isEmptyArray(this.tradeBotBtcBook)}" .items="${this.tradeBotBtcBook}">
						<vaadin-grid-column
							...
							header="${translate("tradepage.tchange8")} (UNCIA)"
							.renderer=${(root, column, data) => {
								render(html`<span>${data.item.botBtcUnciaAmount}</span>`, root)
							}}
						>
						</vaadin-grid-column>
						<vaadin-grid-column
						...
						<vaadin-grid-column
							...
							header="${translate("tradepage.tchange10")} (${this.listedCoins.get(this.selectedCoin).coinCode})"
							.renderer=${(root, column, data) => {
								const totalCoins = this.round(parseFloat(data.item.botBtcUnciaAmount) * parseFloat(data.item.botBtcPrice))
				...
				<div class="border-wrapper">
					<vaadin-grid theme="compact column-borders row-stripes wrap-cell-content" id="tradeBotLTCGrid" aria-label="Auto Buy LTC" ?hidden="${this.isEmptyArray(this.tradeBotLtcBook)}" .items="${this.tradeBotLtcBook}">
						<vaadin-grid-column
							...
							header="${translate("tradepage.tchange8")} (UNCIA)"
							.renderer=${(root, column, data) => {
								render(html`<span>${data.item.botLtcUnciaAmount}</span>`, root)
							}}
						>
						</vaadin-grid-column>
						<vaadin-grid-column
						...
						<vaadin-grid-column
							...
							header="${translate("tradepage.tchange10")} (${this.listedCoins.get(this.selectedCoin).coinCode})"
							.renderer=${(root, column, data) => {
								const totalCoins = this.round(parseFloat(data.item.botLtcUnciaAmount) * parseFloat(data.item.botLtcPrice))
								const myBotFunds = this.round(parseFloat(this.listedCoins.get(this.selectedCoin).balance))
								if (Number(myBotFunds) > Number(totalCoins)) {
									this.autoBuyBotDisable = false
									render(html`<span style="color: rgba(55, 160, 51);">${totalCoins}</span>`, root)
				...
				<div class="border-wrapper">
					<vaadin-grid theme="compact column-borders row-stripes wrap-cell-content" id="tradeBotDOGEGrid" aria-label="Auto Buy DOGE" ?hidden="${this.isEmptyArray(this.tradeBotDogeBook)}" .items="${this.tradeBotDogeBook}">
						<vaadin-grid-column
							...
							header="${translate("tradepage.tchange8")} (UNCIA)"
							.renderer=${(root, column, data) => {
								render(html`<span>${data.item.botDogeUnciaAmount}</span>`, root)
							}}
						>
						</vaadin-grid-column>
						<vaadin-grid-column
						...
						<vaadin-grid-column
							...
							header="${translate("tradepage.tchange10")} (${this.listedCoins.get(this.selectedCoin).coinCode})"
							.renderer=${(root, column, data) => {
								const totalCoins = this.round(parseFloat(data.item.botDogeUnciaAmount) * parseFloat(data.item.botDogePrice))
								const myBotFunds = this.round(parseFloat(this.listedCoins.get(this.selectedCoin).balance))
								if (Number(myBotFunds) > Number(totalCoins)) {
									this.autoBuyBotDisable = false
									render(html`<span style="color: rgba(55, 160, 51);">${totalCoins}</span>`, root)
				...
				<div class="border-wrapper">
					<vaadin-grid theme="compact column-borders row-stripes wrap-cell-content" id="tradeBotDGBGrid" aria-label="Auto Buy DGB" ?hidden="${this.isEmptyArray(this.tradeBotDgbBook)}" .items="${this.tradeBotDgbBook}">
						<vaadin-grid-column
							...
							header="${translate("tradepage.tchange8")} (UNCIA)"
							.renderer=${(root, column, data) => {
								render(html`<span>${data.item.botDgbUnciaAmount}</span>`, root)
							}}
						>
						</vaadin-grid-column>
						<vaadin-grid-column
						...
						<vaadin-grid-column
							...
							header="${translate("tradepage.tchange10")} (${this.listedCoins.get(this.selectedCoin).coinCode})"
							.renderer=${(root, column, data) => {
								const totalCoins = this.round(parseFloat(data.item.botDgbUnciaAmount) * parseFloat(data.item.botDgbPrice))
								const myBotFunds = this.round(parseFloat(this.listedCoins.get(this.selectedCoin).balance))
								if (Number(myBotFunds) > Number(totalCoins)) {
									this.autoBuyBotDisable = false
									render(html`<span style="color: rgba(55, 160, 51);">${totalCoins}</span>`, root)
				...
				<div class="border-wrapper">
					<vaadin-grid theme="compact column-borders row-stripes wrap-cell-content" id="tradeBotRVNGrid" aria-label="Auto Buy RVN" ?hidden="${this.isEmptyArray(this.tradeBotRvnBook)}" .items="${this.tradeBotRvnBook}">
						<vaadin-grid-column
							...
							header="${translate("tradepage.tchange8")} (UNCIA)"
							.renderer=${(root, column, data) => {
								render(html`<span>${data.item.botRvnUnciaAmount}</span>`, root)
							}}
						>
						</vaadin-grid-column>
						<vaadin-grid-column
						...
						<vaadin-grid-column
							...
							header="${translate("tradepage.tchange10")} (${this.listedCoins.get(this.selectedCoin).coinCode})"
							.renderer=${(root, column, data) => {
								const totalCoins = this.round(parseFloat(data.item.botRvnUnciaAmount) * parseFloat(data.item.botRvnPrice))
								const myBotFunds = this.round(parseFloat(this.listedCoins.get(this.selectedCoin).balance))
								if (Number(myBotFunds) > Number(totalCoins)) {
									this.autoBuyBotDisable = false
									render(html`<span style="color: rgba(55, 160, 51);">${totalCoins}</span>`, root)
				...
				<div class="border-wrapper">
					<vaadin-grid theme="compact column-borders row-stripes wrap-cell-content" id="tradeBotARRRGrid" aria-label="Auto Buy ARRR" ?hidden="${this.isEmptyArray(this.tradeBotArrrBook)}" .items="${this.tradeBotArrrBook}">
						<vaadin-grid-column
							...
							header="${translate("tradepage.tchange8")} (UNCIA)"
							.renderer=${(root, column, data) => {
								render(html`<span>${data.item.botArrrUnciaAmount}</span>`, root)
							}}
						>
						</vaadin-grid-column>
						<vaadin-grid-column
						...
						<vaadin-grid-column
							...
							header="${translate("tradepage.tchange10")} (${this.listedCoins.get(this.selectedCoin).coinCode})"
							.renderer=${(root, column, data) => {
								const totalCoins = this.round(parseFloat(data.item.botArrrUnciaAmount) * parseFloat(data.item.botArrrPrice))
								const myBotFunds = this.round(parseFloat(this.listedCoins.get(this.selectedCoin).balance))
								if (Number(myBotFunds) > Number(totalCoins)) {
									this.autoBuyBotDisable = false
									render(html`<span style="color: rgba(55, 160, 51);">${totalCoins}</span>`, root)
    ...
    render() {
		...
		<div id="trade-portal-page">
			...
			<div style="min-height: 50px; display: flex; padding-bottom: 10px; padding-top: 10px; margin: auto;">
				<h2 style="margin: 0 0 15px 0; line-height: 50px; display: inline;">${translate("info.inf13")} UNCIA ${translate("info.inf14")} ${this.listedCoins.get(this.selectedCoin).coinCode} - &nbsp;</h2>
				<mwc-select outlined id="coinSelectionMenu" label="${translate("tradepage.tchange2")}">
					<mwc-list-item value="BITCOIN"><span class="coinName btc" style="color: var(--black);">BTC / UNCIA</span></mwc-list-item>
					<mwc-list-item value="LITECOIN" selected><span class="coinName ltc" style="color: var(--black);">LTC / UNCIA</span></mwc-list-item>
					<mwc-list-item value="DOGECOIN"><span class="coinName doge" style="color: var(--black);">DOGE / UNCIA</span></mwc-list-item>
					<mwc-list-item value="DIGIBYTE"><span class="coinName dgb" style="color: var(--black);">DGB / UNCIA</span></mwc-list-item>
					<mwc-list-item value="RAVENCOIN"><span class="coinName rvn" style="color: var(--black);">RVN / UNCIA</span></mwc-list-item>
                              <mwc-list-item value="PIRATECHAIN"><span class="coinName arrr" style="color: var(--black);">ARRR / UNCIA</span></mwc-list-item>
				</mwc-select>
			...
			<div id="trade-portal">
				...
				<div style="text-align: center;">
					...
					<h3 style="color: var(--black);">1 <span style="color: #03a9f4;">UNCIA</span> = ${this.exchangeRateUncia()} ${this.listedCoins.get(this.selectedCoin).coinCode}</h3>
				</div>
			</div>
		</div>
		...
		<mwc-dialog style="background: var(--white);" id="tradeBotBTCAddDialog">
			...
			<div class="card-bot" style="display:${this.showAddAutoBuy ? 'flex' : 'none'}">
				<div style="text-align: center;">
					<h1>${this.listedCoins.get(this.selectedCoin).coinCode} ==> UNCIA</h1>
					<hr>
				</div>
				...
				<span class="tab-text">${translate("tradepage.tchange8")} (UNCIA)*</span>
				<p>
					<mwc-textfield
						style="width: 100%; color: var(--black);"
						id="autoBuyBTCUnciaAmountInput"
						required
						label=""
		...
		<mwc-dialog style="background: var(--white);" id="tradeBotDOGEAddDialog">
			...
			<div class="card-bot" style="display:${this.showAddAutoBuy ? 'flex' : 'none'}">
				...
				<p>
					<mwc-textfield
						style="width: 100%; color: var(--black);"
						id="autoBuyDOGEUnciaAmountInput"
						required
						label=""
						placeholder="0.0000000"
						type="number"
		...
		<mwc-dialog style="background: var(--white);" id="tradeBotARRRAddDialog">
			...
			<div class="card-bot" style="display:${this.showAddAutoBuy ? 'flex' : 'none'}">
				...
				<p>
					<mwc-textfield
						style="width: 100%; color: var(--black);"
						id="autoBuyARRRUnciaAmountInput"
						required
						label=""
						placeholder="0.0000000"
						type="number"
    ...
    firstUpdated() {
        ...
        this._openOrdersGrid.querySelector('#unciaAmountColumn').headerRenderer = function (root) {
            const amountString = get("tradepage.tchange8")
            root.innerHTML = '<vaadin-grid-sorter path="unciaAmount">' + amountString + ' (UNCIA)</vaadin-grid-sorter>'
        }

        this._myOrdersGrid = this.shadowRoot.getElementById('myOrdersGrid')

        const getUnciaBtcPrice = () => {
            parentEpml.request("apiCall", { url: `/crosschain/price/BITCOIN?inverse=true` }).then((res) => {
                setTimeout(() => { this.unciabtc = (Number(res) / 1e8).toFixed(8) }, 1)
            })
            setTimeout(getUnciaBtcPrice, 300000)
        }

        const getUnciaLtcPrice = () => {
            parentEpml.request("apiCall", { url: `/crosschain/price/LITECOIN?inverse=true` }).then((res) => {
                setTimeout(() => { this.uncialtc = (Number(res) / 1e8).toFixed(8) }, 1)
            })
            setTimeout(getUnciaLtcPrice, 300000)
        }

        const getUnciaDogePrice = () => {
            parentEpml.request("apiCall", { url: `/crosschain/price/DOGECOIN?inverse=true` }).then((res) => {
                setTimeout(() => { this.unciadoge = (Number(res) / 1e8).toFixed(8) }, 1)
            })
            setTimeout(getUnciaDogePrice, 300000)
        }

        const getUnciaDgbPrice = () => {
            parentEpml.request("apiCall", { url: `/crosschain/price/DIGIBYTE?inverse=true` }).then((res) => {
                setTimeout(() => { this.unciadgb = (Number(res) / 1e8).toFixed(8) }, 1)
            })
            setTimeout(getUnciaDgbPrice, 300000)
        }

        const getUnciaRvnPrice = () => {
            parentEpml.request("apiCall", { url: `/crosschain/price/RAVENCOIN?inverse=true` }).then((res) => {
                setTimeout(() => { this.unciarvn = (Number(res) / 1e8).toFixed(8) }, 1)
            })
            setTimeout(getUnciaRvnPrice, 300000)
        }

        const getUnciaArrrPrice = () => {
            parentEpml.request("apiCall", { url: `/crosschain/price/PIRATECHAIN?inverse=true` }).then((res) => {
                setTimeout(() => { this.unciaarrr = (Number(res) / 1e8).toFixed(8) }, 1)
            })
            setTimeout(getUnciaArrrPrice, 300000)
        }
        ...
        parentEpml.ready().then(() => {
            ...
            parentEpml.subscribe('config', (c) => {
                if (!configLoaded) {
                    setTimeout(getUnciaBtcPrice, 1)
                    setTimeout(getUnciaLtcPrice, 1)
                    setTimeout(getUnciaDogePrice, 1)
                    setTimeout(getUnciaDgbPrice, 1)
                    setTimeout(getUnciaRvnPrice, 1)
                    setTimeout(getUnciaArrrPrice, 1)
                    configLoaded = true
                }
    ...
    exchangeRateUncia() {
        switch(this.listedCoins.get(this.selectedCoin).coinCode) {
            case "BTC":
                this.unciaRatio = this.unciabtc
                break
            case "LTC":
                this.unciaRatio = this.uncialtc
                break
            case "DOGE":
                this.unciaRatio = this.unciadoge
                break
            case "DGB":
                this.unciaRatio = this.unciadgb
                break
            case "RVN":
                this.unciaRatio = this.unciarvn
                break
            case "ARRR":
                this.unciaRatio = this.unciaarrr
                break
            default:
                break
        }
        return html`${this.unciaRatio}`
    }
    ...
    addToBTCTradebook() {
        const addBotBtcUnciaAmount = this.shadowRoot.getElementById('autoBuyBTCUnciaAmountInput').value
        const addBotBtcPrice = this.shadowRoot.getElementById('autoBuyBTCPriceInput').value
        const addBtcUnciaAmount = this.round(parseFloat(addBotBtcUnciaAmount))
        ...
        const newBtcTradebookItem = {
            botBtcUnciaAmount: addBtcUnciaAmount,
    ...
    addToLTCTradebook() {
        const addBotLtcUnciaAmount = this.shadowRoot.getElementById('autoBuyLTCUnciaAmountInput').value
        const addBotLtcPrice = this.shadowRoot.getElementById('autoBuyLTCPriceInput').value
        const addLtcUnciaAmount = this.round(parseFloat(addBotLtcUnciaAmount))
        ...
        const newLtcTradebookItem = {
            botLtcUnciaAmount: addLtcUnciaAmount,
    ...
    addToDOGETradebook() {
        const addBotDogeUnciaAmount = this.shadowRoot.getElementById('autoBuyDOGEUnciaAmountInput').value
        const addBotDogePrice = this.shadowRoot.getElementById('autoBuyDOGEPriceInput').value
        const addDogeUnciaAmount = this.round(parseFloat(addBotDogeUnciaAmount))
        ...
        const newDogeTradebookItem = {
            botDogeUnciaAmount: addDogeUnciaAmount,
    ...
    addToDGBTradebook() {
        const addBotDgbUnciaAmount = this.shadowRoot.getElementById('autoBuyDGBUnciaAmountInput').value
        const addBotDgbPrice = this.shadowRoot.getElementById('autoBuyDGBPriceInput').value
        const addDgbUnciaAmount = this.round(parseFloat(addBotDgbUnciaAmount))
        ...
        const newDgbTradebookItem = {
            botDgbUnciaAmount: addDgbUnciaAmount,
    ...
    addToRVNTradebook() {
        const addBotRvnUnciaAmount = this.shadowRoot.getElementById('autoBuyRVNUnciaAmountInput').value
        const addBotRvnPrice = this.shadowRoot.getElementById('autoBuyRVNPriceInput').value
        const addRvnUnciaAmount = this.round(parseFloat(addBotRvnUnciaAmount))
        ...
        const newRvnTradebookItem = {
            botRvnUnciaAmount: addRvnUnciaAmount,
    ...
    addToARRRTradebook() {
        const addBotArrrUnciaAmount = this.shadowRoot.getElementById('autoBuyARRRUnciaAmountInput').value
        const addBotArrrPrice = this.shadowRoot.getElementById('autoBuyARRRPriceInput').value
        const addArrrUnciaAmount = this.round(parseFloat(addBotArrrUnciaAmount))
        ...
        const newArrrTradebookItem = {
            botArrrUnciaAmount: addArrrUnciaAmount,
    ...
    async setForeignCoin(coin,beingInitialized) {
        ...
        if(beingInitialized){
            ...
            pairIconContainer.setAttribute('style', 'left: 10px;top: 50%;transform: translate(0, -50%);height: 26px;width: 45px;position: absolute;background-repeat: no-repeat;background-size: cover;background-image: url(/img/uncia'+pairicon+'.png);')
            ...
            let pairicon = (_this.listedCoins.get(_this.selectedCoin).coinCode).toLowerCase()
            pairIconContainer.style.backgroundImage='url(/img/uncia'+pairicon+'.png)'
        }
    ...
    checkTradeBotValues() {
        const checkTradeBotAmountInput = this.shadowRoot.getElementById('autoBuy' + this.listedCoins.get(this.selectedCoin).coinCode + 'UnciaAmountInput').value
    ...
    processOfferingTrade(offer) {
        try {
            if(this.listedCoins.get(offer.foreignBlockchain).name!='') {
                const offerItem = {
                    ...offer,
                    unciaAmount: parseFloat(offer.unciaAmount),
                    price: parseFloat(offer.foreignAmount) / parseFloat(offer.unciaAmount),
                }
                const addOffer = () => {
    ...
    processRedeemedTrade(offer) {
        try {
            if (this.listedCoins.get(offer.foreignBlockchain).name!='') {
                ...
                if (offer.aquilaCreator === this.selectedAddress.address) {
                ...
                } else if (offer.partnerAquilaReceivingAddress === this.selectedAddress.address) {
                    if (this.listedCoins.get(offer.foreignBlockchain).tradeOffersSocketCounter > 1) {
                        this.updateWalletBalance()
                    }
                    const offerItem = {
    ...
    processTradingTrade(offer) {
        try {
            if (this.listedCoins.get(offer.foreignBlockchain).name!='') {
                ...
                if (offer.aquilaCreator === this.selectedAddress.address && this.listedCoins.get(offer.foreignBlockchain).tradeOffersSocketCounter > 1) {
                    this.updateWalletBalance()
                }
                this._openOrdersGrid.items.forEach((item, index) => {
                    if (item.aquilaAtAddress === offer.aquilaAtAddress) {
                        this._openOrdersGrid.items.splice(index, 1)
                        this.listedCoins.get(offer.foreignBlockchain).tradeOffersSocketCounter > 1 ? this._openOrdersGrid.clearCache() : null
                    }
                })
                this.listedCoins.get(offer.foreignBlockchain).openOrders = this.listedCoins.get(offer.foreignBlockchain).openOrders.filter((order) => order.aquilaAtAddress !== offer.aquilaAtAddress)
            }
        } catch(e) {
            ...
            const startOfferPresenceMapping = async () => {
                ...
                if (tradePresenceTxns !== null) {
                    await asyncForEach(tradePresenceTxns, async (tradePresence) => {
                        let offerIndex = offeringTrades.findIndex((offeringTrade) => offeringTrade.aquilaCreatorTradeAddress === tradePresence.tradeAddress)
    ...
    updateAccountBalance() {
        ...
        parentEpml.request('apiCall', {
            ...
            .then((res) => {
                this.listedCoins.get("AQUILA").balance = res
    ...
    checkTradeBotAmount(e) {
        ...
        if (targetAmount.length === 0) {
            ...
            const buyTradeBotAmountInput = this.shadowRoot.getElementById('autoBuy' + this.listedCoins.get(this.selectedCoin).coinCode + 'UnciaAmountInput').value
    ...
    clearTradeBotForm() {
        this.shadowRoot.getElementById('autoBuy' + this.listedCoins.get(this.selectedCoin).coinCode + 'UnciaAmountInput').value = this.initialAmount
