  ...
  async setActiveChatHeadUrl(url) {
   ...
   async getPendingGroupInvites() {
    ...
    setChatHeads(chatObj) {
        ...
        let groupList = chatObjGroups.map(group => group.groupId === 0 ? { groupId: group.groupId, url: `group/${group.groupId}`, groupName: "Aquila General Chat", timestamp: group.timestamp === undefined ? 2 : group.timestamp, sender: group.sender } : { ...group, timestamp: group.timestamp === undefined ? 1 : group.timestamp, url: `group/${group.groupId}` })
