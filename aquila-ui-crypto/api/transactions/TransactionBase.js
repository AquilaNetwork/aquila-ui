'use strict'
import { TX_TYPES, UNCIA_DECIMALS } from '../constants.js'
...
export default class TransactionBase {
	...
	constructor() {
		...
		this.tests = [
			...
			() => {
				if (this._fee < 0) {
					return 'Invalid fee: ' + this._fee / UNCIA_DECIMALS
				}
				return true
			},
			() => {
			...
			() => {
				if (!(this._lastReference instanceof Uint8Array && this._lastReference.byteLength == 64)) {
					if (this._lastReference == 0) {
						return 'Invalid last reference. Please ensure that you have at least 0.001 UNCIA for the transaction fee.'
					}
