...
export const cancelAllOffers = async (requestObject) => {
	...
	const getMyOpenOffers = async () => {
		...
		const myOpenTradeOrders = await res.filter(order => order.mode === "OFFERING" && order.aquilaCreator === address)
	...
	myOpenOffers.forEach(async (openOffer) => {
		let unsignedTxn = await deleteTradeOffer({ creatorPublicKey: publicKey, atAddress: openOffer.aquilaAtAddress })
