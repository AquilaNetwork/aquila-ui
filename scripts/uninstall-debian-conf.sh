...
if grep ^ /etc/apt/sources.list /etc/apt/sources.list.d/* | grep aquila.list; then
	sudo rm /etc/apt/sources.list.d/aquila.list;
fi
...
getDesktopEntry=/home/$getSudoUser/.config/autostart/aquila.desktop;
...
appDirectory=/home/$getSudoUser/.config/aquila-ui/;
