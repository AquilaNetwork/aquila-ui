...
class SettingsPage extends connect(store)(LitElement) {
    ...
    render() {
        return html`
            ...
            <mwc-dialog id="importAquilaNodesListDialog">
                <div style="text-align:center">
                ...
                <div style="min-height: 150px; min-width: 500px; box-sizing: border-box; position: relative;">
                    <frag-file-input accept=".nodes" @file-read-success="${(e) => this.importAquilaNodesList(e.detail.result)}"></frag-file-input>
    ...
    removeList() {
        localStorage.removeItem("myAquilaNodes")
        ...
        localStorage.setItem('myAquilaNodes', JSON.stringify(renewNodes))
    ...
    addNode() {
        ...
        if (protocolList.length >= 4 && domainInput.length >= 3 && portInput.length >= 2) {
            ...
            const haveNodes = JSON.parse(localStorage.getItem('myAquilaNodes'))

            if (haveNodes === null || haveNodes.length === 0) {

                var savedNodes = [];
                savedNodes.push(nodeObject);
                localStorage.setItem('myAquilaNodes', JSON.stringify(savedNodes))
                ...
                var stored = JSON.parse(localStorage.getItem('myAquilaNodes'));
                stored.push(nodeObject);
                localStorage.setItem('myAquilaNodes', JSON.stringify(stored));
    ...
    openImportNodesDialog() {
        this.shadowRoot.querySelector("#importAquilaNodesListDialog").show()
    }

    closeImportNodesDialog() {
        this.shadowRoot.querySelector("#importAquilaNodesListDialog").close()
    }

    renderExportNodesListButton() {
        return html`
            <mwc-button dense unelevated label="${translate("settings.export")}" @click="${() => this.exportAquilaNodesList()}"></mwc-button>
        `
    }

    exportAquilaNodesList() {
        const aquilaNodesList = JSON.stringify(localStorage.getItem("myAquilaNodes"))
        const aquilaNodesListSave = JSON.parse((aquilaNodesList) || "[]")
        const blob = new Blob([aquilaNodesListSave], { type: 'text/plain;charset=utf-8' })
        FileSaver.saveAs(blob, `aquila.nodes`)

        let snack4string = get("settings.snack4")
        snackbar.add({
            labelText: `${snack4string} aquila.nodes`,
            dismiss: true
        })
    }
    ...
    async importAquilaNodesList(file) {
        localStorage.removeItem("myAquilaNodes")
        const newItems = JSON.parse((file) || "[]")
        localStorage.setItem("myAquilaNodes", JSON.stringify(newItems))
        this.shadowRoot.querySelector('#importAquilaNodesListDialog').close()
